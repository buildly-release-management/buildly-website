...
SERVER_SCRIPT="$PROJECT_ROOT/ForgeWeb/admin/file-api.py"
SERVER_DIR="$PROJECT_ROOT/ForgeWeb/admin"
DEFAULT_PORT=8000
...
get_server_pid() {
    ...
    local pid=$(pgrep -f "python.*file-api.py" 2>/dev/null | head -1)
    if [[ -n "$pid" ]]; then
        echo "$pid"
        return 0
    fi
...
start_server() {
    ...
    # Change to ForgeWeb admin dir (needed for relative imports in file-api.py)
    cd "$SERVER_DIR"
    
    # Start server in background
    print_info "Launching ForgeWeb server..."
    nohup "$VENV_DIR/bin/python" "$SERVER_SCRIPT" --port "$port" > "$LOG_FILE" 2>&1 &
    local pid=$!
    ...
    if ps -p "$pid" > /dev/null 2>&1; then
        ...
        echo -e "  ${GREEN}→ Article Editor:       http://localhost:$port/admin/editor.html${NC}"
        echo -e "  ${GREEN}→ Page Editor:          http://localhost:$port/admin/page-editor.html${NC}"
        echo -e "  ${GREEN}→ Navigation Manager:   http://localhost:$port/admin/navigation-manager.html${NC}"
        echo -e "  ${GREEN}→ Branding:             http://localhost:$port/admin/branding-manager.html${NC}"
        echo -e "  ${GREEN}→ Deploy to GitHub:     http://localhost:$port/admin/deploy.html${NC}"
        echo -e "  ${GREEN}→ Preview Site:         http://localhost:$port/${NC}"
        echo ""
